...
var JotForm = {
    ...
    takeScreenShot: function(id){
        ...
        //this.wishboxServer = 'http://ec2-107-22-70-25.compute-1.amazonaws.com/wishbox-bot.php'; 
        this.wishboxServer = 'http://screenshots.jotform.com/wishbox-server.php';
        //this.wishboxServer = "http://beta23.jotform.com/server.php";//JotForm.server;
...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
    ...
    */
                ...
                try{
                    ...
                    switch (question.type) {
                        ...
                        case "control_grading":
                            ...
                            var boxes = document.getElementsByName("q" + qid + "_clickTo" + "[]");
                        ...
                        case "control_fullname":
                            $H(question.items).each(function(item) {
                                if($(item.key + "_" + qid)) {($(item.key + "_" + qid).value = item.value);
                                }
                            });
                            break;
                        case "control_phone":
                        ...
                        case "control_hidden":
                            if($('input_' + qid)) {
                                if(JotForm.saveForm || document.get.mode == 'edit') {
                                    ...
                                    sec.insert({
                                        top : '<li id="id_' + qid + '" class="form-line" title="Hidden Field">' + '<label for="input_' + qid + '" id="label_' + qid + '" class="form-label-left"> ' + question.text + ' </label>' + '<div class="form-input" id="cid_' + qid + '"></div></li>'
                                    });
                        ...
                        case 'control_authnet':
                            ...
                            break;
                        default:
                            if($('input_' + qid)) {($('input_' + qid).putValue(question.value));
                            }
                            break;
                    }
    ...
    showField: function(field){
        ...
        $H(JotForm.defaultValues).each(function(pair){
            if(pair.key.startsWith('input_'+field)){
                if($(pair.key).hasClassName('form-radio') || $(pair.key).hasClassName('form-checkbox')){
        ...
        if($('input_'+field) && $('input_'+field).hinted){
            $('input_'+field).runHint();
        }
        ...
        try {
            $A(JotForm.conditions).each(function(condition){
                ...
                if (condition.type == 'field') {
                    ...
                    if (condition.action.visibility.toLowerCase() == 'show') {
                        if($('id_' + condition.action.field)){ $('id_' + condition.action.field).hide(); }
                    } else {
                        if($('id_' + condition.action.field)){ $('id_' + condition.action.field).show(); }
                    }
        ...
        allButtons.each(function(b) {
            if(b.up('ul.form-section')) {
                if(b.up('ul.form-section').style.display == "none") {
                    ...
                    if(b.className != "form-submit-button disabled") {
                        b.enable();
                    }
                }
            }
